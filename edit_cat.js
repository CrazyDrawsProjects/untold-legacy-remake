...
var initJSON = function() {
  ...
  cat_id = urlParam("catId");
  local_cat = catsJSON.cats[cat_id];

  if (!local_cat) {
    console.error("Error: Invalid cat_id or local_cat is undefined");
    return;
  }
...
var getCatPhotos = function() {
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#cat_photo_input").val(local_cat.picture);

      if (local_cat.picture.includes("user_uploads"))
      {
        $("#cat_photo_input").append("<option class='user_upload_class' value='"+local_cat.picture+"'>"+local_cat.picture+"</option>");
        $("#cat_photo_input").val(local_cat.picture);
      }
    },
    error: function(data) {
...
var updateHTMLfromCatToAdd = function() {
  $("#cat_photo_img").attr('src',"./img/cat_photos/"+local_cat.picture);
  $("#cat_photo_input").val(local_cat.picture);

  $("#cat_name_input").val(local_cat.name);

  $("#cat_name_title").text(local_cat.name);

  $("#cat_description_input").val(local_cat.color);

  $("#cat_additional_descriptor_input").val(local_cat.additional_descriptor);

  $("#cat_gender_input").val(local_cat.sex);
  ...
  $("#cat_name_color_input").val(local_cat.name_color);

  $("#cat_allegience_input").val(local_cat.allegience);

  $("#cat_mother_type_input").val(local_cat.mother_type);
  $("#cat_father_type_input").val(local_cat.father_type);
  ...
  if (local_cat.mother_type == "living")
  {
    for (var i = 0; i < catsJSON.cats.length; i++)
    {
      ...
      if (local_parent.name != 'deleted' && local_parent.moons > 6 && local_parent.sex == 'she-cat' && local_parent.allegience == local_cat.allegience)
      {
    ...
    for (var i = 0; i < dead_mothers.cats.length; i++)
    {
      ...
      if (local_parent.name != 'deleted' && local_parent.allegience == local_cat.allegience)
      {
  ...
  if (local_cat.father_type == "living")
  {
    for (var i = 0; i < catsJSON.cats.length; i++)
    {
      //start with my clan only
      var local_parent = catsJSON.cats[i];
      if (local_parent.name != 'deleted' && local_parent.moons > 6 && local_parent.sex == 'tom' && local_parent.allegience == local_cat.allegience)
      {
  ...
  $("#cat_mother_input").val(local_cat.mother);
  $("#cat_father_input").val(local_cat.father);


  $("#cat_moons_input").val(local_cat.moons);
  ...
  for (var i = 0; i < catsJSON.cats.length; i++)
  {
    ...
    if (local_apprentice.name != 'deleted' && local_apprentice.rank == 'apprentice' && local_apprentice.allegience == local_cat.allegience)
    {
  ...
  $("#cat_apprentice_input").val(local_cat.apprentice);
  ...
  if (local_cat.allegience != 'Loner')
  {
    ...
    if (local_cat.rank == 'loner' || local_cat.rank == 'kittypet')
    {
      local_cat.rank = 'warrior';
    }
  }
  else {
    $('#cat_rank_input option').remove();
    ...
    if (local_cat.rank != 'loner' && local_cat.rank != 'kittypet')
    {
      local_cat.rank = 'loner';
    }
  }

  $("#cat_rank_input").val(local_cat.rank);
}

var updateCatToAdd = function() {
  local_cat.name = $("#cat_name_input").val();
  local_cat.color = $("#cat_description_input").val();
  local_cat.additional_descriptor = $("#cat_additional_descriptor_input").val();

  local_cat.sex = $("#cat_gender_input").val();

  local_cat.rank = $("#cat_rank_input").val();

  local_cat.allegience = $("#cat_allegience_input").val();

  local_cat.name_color = $("#cat_name_color_input").val();

  local_cat.moons = $('#cat_moons_input').val();

  local_cat.apprentice = $("#cat_apprentice_input").val();

  local_cat.mother = $("#cat_mother_input").val();
  local_cat.father = $("#cat_father_input").val();

  local_cat.picture = $("#cat_photo_input").val();

  if (local_cat.mother_type !== $("#cat_mother_type_input").val())
  {
    local_cat.mother = "Unknown";
  }
  if (local_cat.father_type !== $("#cat_father_type_input").val())
  {
    local_cat.father = "Unknown";
  }

  local_cat.mother_type = $("#cat_mother_type_input").val();
  local_cat.father_type = $("#cat_father_type_input").val();



  if (local_cat.mother == '') { local_cat.mother = 'Unknown'; }
  if (local_cat.father == '') { local_cat.father = 'Unknown'; }
  ...
  if (local_cat.name == '')
  {
    local_cat.name = 'NewCat';
    $("#cat_name_input").val("NewCat");
  }

  if (local_cat.rank == 'apprentice' || local_cat.rank == 'kitten')
  {
    local_cat.apprentice = -1;
  }
  ...
  fileReader.onload = function () {
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            {
              local_cat.picture = responseData.new_file_name;
...
var randomizeCat = function()
{
  ...
  local_cat.allegience = possible_allegiences[Math.floor(Math.random()*possible_allegiences.length)];
  ...
  local_cat.rank = possible_ranks[Math.floor(Math.random()*possible_ranks.length)];
  ...
  if (local_cat.allegience == "Loner")
  {
    ...
    else if (local_cat.rank == "kittypet")
    {
      cat_suffix = "";
    }
  ...
  local_cat.name = cat_prefix + cat_suffix;

  local_cat.moons = Math.floor(Math.random()*100)+12;

  if (local_cat.rank == 'leader' || local_cat.rank == 'deputy' && local_cat.moons < 30)
  {
    local_cat.moons += 30;
  }
  if (local_cat.rank == "kitten")
  {
    local_cat.moons = Math.floor(Math.random()*7);
  }
  if (local_cat.rank == "apprentice")
  {
    local_cat.moons = Math.floor(Math.random()*7)+6;
  }
  if (local_cat.rank == "elder")
  {
    local_cat.moons = Math.floor(Math.random()*70)+90;
  }


  var possible_sexes = ["tom", "she-cat"];

  local_cat.sex = possible_sexes[Math.floor(Math.random()*possible_sexes.length)];
  ...
  local_cat.color = possible_colors[Math.floor(Math.random()*possible_colors.length)];

  local_cat.additional_descriptor = "";
  local_cat.flavor_descriptor = "";

  if (Math.random() > 0.7)
  {
    flavor_desc = possible_flavor_descriptors[Math.floor(Math.random()*possible_flavor_descriptors.length)];

    if (flavor_desc == "beautiful" && local_cat.sex == "tom") { flavor_desc = ""; }
    if (flavor_desc == "pretty" && local_cat.sex == "tom") { flavor_desc = ""; }
    if (flavor_desc == "handsome" && local_cat.sex == "she-cat") { flavor_desc = ""; }
  }

  if (Math.random() > 0.7)
  {
    local_cat.additional_descriptor = possible_additional_descriptors[Math.floor(Math.random()*possible_additional_descriptors.length)];
  }

  var save_name_color = getCatNameColor(local_cat.color);

  local_cat.name_color = getCatNameColorNoUnderscores(local_cat.color);

  var spacing = "";
  if (flavor_desc.length > 0) { spacing = " ";}
  local_cat.color = flavor_desc + spacing + local_cat.color;

  var color_category_name = save_name_color;
  
  var local_cat = catsJSON.cats[cat_id];

  local_cat.picture = "color_"+color_category_name+(Math.floor(Math.random()*10)+1)+".jpg";
...
var updateMotherList = function() {
  ...
  if (local_cat.mother_type === "living") {
    for (var i = 0; i < catsJSON.cats.length; i++) {
      var local_parent = catsJSON.cats[i];
      if (local_parent.name !== 'deleted' && local_parent.moons > 6 && local_parent.sex === 'she-cat') {
        var optionHTML = getIcon(local_parent.allegience) + local_parent.name;
        $("#cat_mother_input").append("<option id='mother_" + i + "' value='" + i + "'>" + optionHTML + "</option>");
      }
    }
  } else {
    for (var i = 0; i < dead_mothers.cats.length; i++) {
      var local_parent = dead_mothers.cats[i];
      var optionHTML = getIcon(local_parent.allegience) + local_parent.name + " &dagger;";
      $("#cat_mother_input").append("<option id='dead_mother_" + i + "' value='" + i + "'>" + optionHTML + "</option>");
    }
  }
};

var updateFatherList = function() {
  $("#cat_father_input option").remove();
  $("#cat_father_input").append("<option value='Unknown'>Unknown</option>");
  ...
  if (local_cat.father_type === "living") {
    for (var i = 0; i < catsJSON.cats.length; i++) {
      var local_parent = catsJSON.cats[i];
      if (local_parent.name !== 'deleted' && local_parent.moons > 6 && local_parent.sex === 'tom') {
        var optionHTML = getIcon(local_parent.allegience) + local_parent.name;
        $("#cat_father_input").append("<option id='father_" + i + "' value='" + i + "'>" + optionHTML + "</option>");
      }
    }
  } else {
    for (var i = 0; i < dead_fathers.cats.length; i++) {
      var local_parent = dead_fathers.cats[i];
      var optionHTML = getIcon(local_parent.allegience) + local_parent.name + " &dagger;";
      $("#cat_father_input").append("<option id='dead_father_" + i + "' value='" + i + "'>" + optionHTML + "</option>");
    }
  }
};

var updateCatToEdit = function() {
  if (!local_cat) {
    console.error("Error: local_cat is undefined. Cannot update.");
    return;
  }
  ...
  local_cat.allegience = $("#cat_allegience_input").val();
  local_cat.name_color = $("#cat_name_color_input").val();
  local_cat.moons = parseInt($("#cat_moons_input").val(), 10);
  local_cat.apprentice = $("#cat_apprentice_input").val();
  local_cat.mother = $("#cat_mother_input").val();
  local_cat.father = $("#cat_father_input").val();
  local_cat.picture = $("#cat_photo_input").val();
  ...
  console.log("local_cat updated:", local_cat);
};
